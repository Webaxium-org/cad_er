...
export function calculateTableData(purpose) {
  ...
  const diff = finalRl - Number((purpose.surveyId && purpose.surveyId.reducedLevel) || 0);
...
export default function FieldBookTable({ tableData = [], isEditing = false, onFieldChange = () => {} }) {
  ...
  const renderEditable = (row, key, value) => {
    ...
    return (
      <BasicInput
        value={value ?? ''}
        onChange={(e) => onFieldChange(row.rowIndex, key, nestedIndex, e.target.value)}
        sx={{ minWidth: '90px' }}
      />
    );
  };
  ...
  return (
    <Table size="small">
      <TableHead sx={{ backgroundColor: '#f4f6f8' }}>
        <TableRow>
          {head.map((h) => (
            <TableCell key={h} sx={{ fontWeight: 700 }} align={h === 'CH' ? 'left' : 'right'}>
              {h}
            </TableCell>
          ))}
        </TableRow>
      ...
      <TableBody>
        {tableData.map((row, idx) => (
          <TableRow key={idx}>
            ...
            <TableCell align="right">{renderEditable(row, 'BS', row.BS)}</TableCell>
            <TableCell align="right">{renderEditable(row, 'IS', row.IS)}</TableCell>
            <TableCell align="right">{renderEditable(row, 'FS', row.FS)}</TableCell>
            <TableCell align="right">{row.HI}</TableCell>
            <TableCell align="right">{row.RL}</TableCell>
            <TableCell align="right">{renderEditable(row, 'Offset', row.Offset)}</TableCell>
            <TableCell
              align="right"
              sx={{
                color:
